...
For projects with many components, reading barrel exports, individual source files, story files, the Figma map, and the token map on every skill run adds up (~85 file operations for a 50-component DS). The **DS Registry** (`.claude/ds-registry.json`) collapses all of this into a single JSON file that every skill can load in one read. It is generated by a TypeScript script and auto-syncs via a `prestorybook` npm hook. See the [DS Registry Guide](guides/ds-registry.md) for setup and the [generation script](scripts/generate-ds-registry.ts) for implementation.
